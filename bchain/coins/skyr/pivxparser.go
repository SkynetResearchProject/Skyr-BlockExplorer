...
const (
    // Net Magics
    MainnetMagic wire.BitcoinNet = 0x52594b53
    TestnetMagic wire.BitcoinNet = 0x54534554
    ...
    // Staking Addresses
    STAKING_ADDR_MAIN = 63  // 'S'
    STAKING_ADDR_TEST = 73  // 'W'
...
func init() {
    // SKYR mainnet Address encoding magics
    MainNetParams = chaincfg.MainNetParams
    MainNetParams.Net = MainnetMagic
    MainNetParams.PubKeyHashAddrID = []byte{25} // starting with 'B'
    MainNetParams.ScriptHashAddrID = []byte{28}
    MainNetParams.PrivateKeyID = []byte{153}

    // SKYR testnet Address encoding magics
    TestNetParams = chaincfg.TestNet3Params
    TestNetParams.Net = TestnetMagic
    TestNetParams.PubKeyHashAddrID = []byte{111} // starting with 'm' or 'n'
    TestNetParams.ScriptHashAddrID = []byte{196}
    TestNetParams.PrivateKeyID = []byte{239}
}

// SkyrParser handle
type SkyrParser struct {
    *btc.BitcoinParser
    baseparser                         *bchain.BaseParser
    BitcoinOutputScriptToAddressesFunc btc.OutputScriptToAddressesFunc
}

// NewSkyrParser returns new SkyrParser instance
func NewSkyrParser(params *chaincfg.Params, c *btc.Configuration) *SkyrParser {
    p := &SkyrParser{
...
// GetChainParams contains network parameters for the main Skyr network
...
func (p *SkyrParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
    return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *SkyrParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
    return p.baseparser.UnpackTx(buf)
}

// ParseTx parses byte array containing transaction and returns Tx struct
func (p *SkyrParser) ParseTx(b []byte) (*bchain.Tx, error) {
    t := wire.MsgTx{}
    r := bytes.NewReader(b)
...
func (p *SkyrParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
...
func (p *SkyrParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
    var tx bchain.Tx
...
func (p *SkyrParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
...
func (p *SkyrParser) IsAddrDescIndexable(addrDesc bchain.AddressDescriptor) bool {
...
func (p *SkyrParser) GetAddrDescForUnknownInput(tx *bchain.Tx, input int) bchain.AddressDescriptor {
    if len(tx.Vin) > input {
...
func (p *SkyrParser) GetValueSatForUnknownInput(tx *bchain.Tx, input int) *big.Int {
    if len(tx.Vin) > input {
        scriptHex := tx.Vin[input].ScriptSig.Hex
        if scriptHex != "" {
            script, _ := hex.DecodeString(scriptHex)
...
func (p *SkyrParser) GetValueSatFromZerocoinSpend(signatureScript []byte) (*big.Int, error) {
...
func (p *SkyrParser) P2CSScriptToAddress(script []byte) ([]string, bool, error) {
    if len(script) < 50 {
...
func (p *SkyrParser) P2EXScriptToAddress(script []byte) ([]string, bool, error) {
	if !isP2EXScript(script) {
...
func (p *SkyrParser) GetAddrDescFromAddress(address string) (bchain.AddressDescriptor, error) {
